<!doctype html>
	<html lang="en">
	<head>
		<title>Debit Note {{$serialPrefix.$serialNumber}} {{ucfirst($copy)}}</title>
		<link href="{{ asset('/css/debitNoteTemplate/template1.css') }}" rel="stylesheet">
	</head>
	<body class="template_2">
		<htmlpageheader name="debitNoteHeader">
			<div id="top_header">
				@if($debitNoteStatus === 'quote')
				Quotation
				@else
		...
		<htmlpagefooter name="debitNoteFooter">
			Page {PAGENO} of {nbpg}
		</htmlpagefooter>
		<div>
			<table class="intro_info no_border">
				<tbody>			
					<tr class="debitNote_for_row no_border">
    						@if($copy === 'original')
    						    <td class="debitNote_for" colspan="4" ><img src="{{asset('img/invoice/Original.png')}}" height="50px" /></td>
    						@elseif($copy === 'duplicate')
						        <td class="debitNote_for" colspan="4"><img src="{{asset('img/invoice/Duplicate.png')}}" height="50px" /></td>
						    @elseif($copy === 'triplicate')
						        <td class="debitNote_for" colspan="4"><img src="{{asset('img/invoice/Triplicate.png')}}" height="50px" /></td>
						    @endif
					</tr>
					<tr class="debitNote_for_row no_border " >
	            	...
	            	<td style="width: 30%">
	            		<table class="no_border no_row_border">
	            			<tbody>
	            				...
	            				<tr>
	            					<td colspan="3"><b>{{$dealer['name']}}<br />{{$dealer['billingAddress']}}</b></td>
	            				</tr>
	            				<tr class="">
	            					<td>GSTIN:</td>
